...
describe("computeUserDeptFreeApprovalZone", () => {
  ...
  beforeAll(async () => {
    const parent = await createProductionDept({
      productionId: prodId,
      name: "权限继承父部门",
      permissions: [asPerm("node:milestone/*@create"), asPerm("node:announcement/*@create")],
    });
    ...
    const child = await createProductionDept({
      ...
      permissions: [asPerm("node:member/*/meta@view")],
    });
    childDeptId = child.id;
  ...
  it("member of child dept inherits child + parent permissions", async () => {
    ...
    expect(zone.has(asPerm("node:member/*/meta@view"))).toBe(true);  // child's own
    expect(zone.has(asPerm("node:milestone/*@create"))).toBe(true);      // inherited from parent
    expect(zone.has(asPerm("node:announcement/*@create"))).toBe(true);  // inherited from parent
  });
  ...
  it("POC user gets descendant union + extra, minus blocked", async () => {
    ...
    expect(zone.has(asPerm("node:announcement/*@create"))).toBe(true);
    // POC zone includes child's permissions
    expect(zone.has(asPerm("node:member/*/meta@view"))).toBe(true);
...
describe("revokeGrantsForDeptRemoval: atomic_permission_grant", () => {
  ...
  beforeAll(async () => {
    ...
    const deptB = await createProductionDept({
      productionId: prodId,
      name: "撤销测试部门B",
      permissions: [asPerm("node:announcement/*@create")],
    });
    deptIdB = deptB.id;
    ...
    await getPool().query(
      `INSERT INTO atomic_permission_grant
       ...
       VALUES
         ($1, $2, 'node:milestone/*@create',      'self_confirmed'),
         ($1, $2, 'node:announcement/*@create',  'self_confirmed'),
         ($1, $2, 'node:milestone/*@delete',   'self_confirmed')
  ...
  it("removing from deptA revokes grants no longer in zone (script:manage)", async () => {
    ...
    expect(byKey[asPerm("node:milestone/*@create")]).toBe(true);      // was only in deptA's zone
    expect(byKey[asPerm("node:announcement/*@create")]).toBe(false);  // still in deptB's zone
    expect(byKey[asPerm("node:milestone/*@delete")]).toBe(true);    // never in any zone
  });
  ...
  it("non-self_confirmed grants are not revoked on dept removal", async () => {
    ...
    await getPool().query(
      `INSERT INTO atomic_permission_grant
         (production_id, user_id, permission_key, grant_source)
       VALUES ($1, $2, 'node:announcement/*@edit', 'approval')
       ON CONFLICT DO NOTHING`,
      [prodId, TEST_USER],
    );
    ...
    const { rows } = await getPool().query<{ is_revoked: boolean }>(
      "SELECT is_revoked FROM atomic_permission_grant WHERE production_id = $1 AND user_id = $2 AND permission_key = 'node:announcement/*@edit'",
      [prodId, TEST_USER],
    );
...
describe("revokeGrantsForPocLoss: atomic_permission_grant", () => {
  ...
  beforeAll(async () => {
    ...
    await getPool().query(
      `INSERT INTO atomic_permission_grant
         (production_id, user_id, permission_key, grant_source)
       VALUES
         ($1, $2, 'node:member/*/meta@view', 'self_confirmed'),
         ($1, $2, 'node:milestone/*@create',    'self_confirmed')
       ON CONFLICT DO NOTHING`,
      [prodId, EXTRA_USER_2],
    );
  ...
  it("losing POC status revokes grants that were only in POC zone", async () => {
    ...
    expect(byKey[asPerm("node:milestone/*@create")]).toBe(false);    // still in member zone (memberDept)
    // cue_list:view: user is still in pocDept as regular member, so still in zone
    expect(byKey[asPerm("node:member/*/meta@view")]).toBe(false);
  });
});
...
describe("recomputeAndRevokeGrants: role_change via setMemberRoles", () => {
  ...
  beforeAll(async () => {
    ...
    await setRolePermissions(roleAlphaId, [asPerm("node:announcement/*@create")]);

    const beta = await createProductionRole(prodId, `test-role-beta-${shortId()}`);
    roleBetaId = beta.id;
    roleBetaName = beta.name;
    await setRolePermissions(roleBetaId, [asPerm("node:milestone/*@create")]);
  ...
  it("removing role-alpha revokes its exclusive grants but keeps role-beta grants", async () => {
    ...
    const byKey = Object.fromEntries(rows.map((r) => [r.permission_key, r.is_revoked]));
    expect(byKey[asPerm("node:announcement/*@create")]).toBe(true);  // was only in role-alpha zone
    expect(byKey[asPerm("node:milestone/*@create")]).toBe(false);      // still in role-beta zone
    expect(byKey[asPerm("node:member/*/meta@view")]).toBe(true);    // never in any role zone
  });
  ...
  it("non-self_confirmed grants are never revoked on role change", async () => {
    ...
    const { rows } = await getPool().query<{ is_revoked: boolean }>(
      `SELECT is_revoked FROM atomic_permission_grant
       WHERE production_id = $1 AND user_id = $2 AND permission_key = 'node:announcement/*@edit'`,
      [prodId, EXTRA_USER_1],
    );
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      `INSERT INTO atomic_permission_grant
       ...
       VALUES
         ($1, $2, 'node:milestone/*@create',   'self_confirmed'),
         ($1, $2, 'node:announcement/*@edit', 'approval'),
         ($1, $2, 'node:milestone/*@edit', 'direct')
       ON CONFLICT DO NOTHING`,
      [prodId, EXTRA_USER_1],
    );
  });
...
describe("updateProductionDept: permissions narrowing cascades to members", () => {
  ...
  beforeAll(async () => {
    ...
    const narrowingDept = await createProductionDept({
      ...
      permissions: [asPerm("node:announcement/*@create"), asPerm("node:member/*/meta@view")],
    });
  ...
  it("narrowing dept permissions revokes grants no longer in any zone", async () => {
    // Remove 'node:member/*/meta@view' from the dept's permissions
    ...
    const byKey = Object.fromEntries(rows.map((r) => [r.permission_key, r.is_revoked]));
    expect(byKey[asPerm("node:announcement/*@create")]).toBe(false);  // still in narrowed dept zone
    expect(byKey[asPerm("node:member/*/meta@view")]).toBe(true);       // dept no longer grants this
    expect(byKey[asPerm("org:assign_member")]).toBe(true);   // was never in any dept zone
  });
});
