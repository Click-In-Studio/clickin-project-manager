...
// 协作远端光标的块坐标（#517）：两端过一次 markdown 后块序号必须仍能对齐。
//
// 断言的不是「序号算成几」，而是**发送端光标所在的文字，接收端解析出的位置也在
// 同一段文字里**——中间真的走一遍 getMarkdown → 重新解析，和线上 update 帧同款。
// 空行也过得了 markdown（#576 空段落方言）之后，发送端光标停在空行上，接收端也
// 落在那个空行上，而不是被投影到隔壁段。
...
function senderCursor(e: Editor, localIndex: number, offset: number) {
  ...
  return { blockIndex: $head.index(0), offset: Math.max(0, $head.pos - $head.start(1)) };
}

/** 接收端：解析发送端的 markdown，把远端坐标落成位置，回报所在顶层块的序号、文字与块内偏移 */
function receiverLanding(md: string, cursor: { blockIndex: number; offset: number }) {
  ...
  return { index: $pos.index(0), text: $pos.node(1).textContent, offset: pos - $pos.start(1), childCount: r.state.doc.childCount };
}

describe("发送端 → markdown 往返 → 接收端落点", () => {
  it("光标前有空行：空行过得了 markdown，对方块数一致，落到同一段文字", () => {
    const s = makeEditor(doc(P("a"), P(), P(), P("bcd")));
    const cursor = senderCursor(s, 3, 2);
    expect(cursor).toEqual({ blockIndex: 3, offset: 2 });
    const landed = receiverLanding(toMarkdown(s), cursor);
    expect(landed.childCount).toBe(4);
    expect(landed).toMatchObject({ index: 3, text: "bcd", offset: 2 });
  });

  it("文末回车停在空行：对方也有这一块，落在空行上", () => {
    const s = makeEditor(doc(P("abc"), P()));
    const cursor = senderCursor(s, 1, 0);
    expect(receiverLanding(toMarkdown(s), cursor)).toMatchObject({ index: 1, text: "", offset: 0, childCount: 2 });
  });

  it("光标停在两段之间的空行：落在那个空行，不投影到下一段", () => {
    const s = makeEditor(doc(P("a"), P(), P("b")));
    expect(receiverLanding(toMarkdown(s), senderCursor(s, 1, 0))).toMatchObject({ index: 1, text: "" });
  });

  it("空标题/空列表这类有形态的块同样按序号对齐", () => {
    const s = makeEditor(doc(P("a"), { type: "heading", attrs: { level: 2 } }, P(), P("b")));
    expect(receiverLanding(toMarkdown(s), senderCursor(s, 3, 1))).toMatchObject({ index: 3, text: "b", offset: 1 });
  });
  ...
  it("列表内光标：偏移跨嵌套展平，对方落在同一项文字里", () => {
    ...
    expect(cursor.blockIndex).toBe(2);
...
describe("resolveRemoteCursorPos 兜底", () => {
  ...
  it("整篇都是空行也能落到一个合法位置", () => {
    ...
    const pos = resolveRemoteCursorPos(r.state.doc, { blockIndex: 1, offset: 0 });
    expect(() => r.state.doc.resolve(pos)).not.toThrow();
    expect(r.state.doc.resolve(pos).index(0)).toBe(1);
  });
});
