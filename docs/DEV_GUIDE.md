...
- **门语境一律用 `hasEffectiveGrant` / `hasAnyEffectiveGrant`**（自带 owner 旁路）。裸 `hasGrant` / `hasAnyGrant` / `listGrantedResourceIds` 不含任何旁路，只在明确「能力票不该给 owner 旁路」的场景使用并写注释。 `app/` `components/` 下的裸调用有逐文件记账棘轮（`tests/perm/bare-grant-ratchet.test.ts`，#604）：只降不升，收敛一处就把记账值改下来，新增一处即红。
