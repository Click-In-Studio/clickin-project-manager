...
固定规则执行后，空间仍不足时必须按以下顺序处理，只有当前步骤仍无法容纳时才进入下一步：

1. 折叠搜索框文字，仅保留搜索图标。
2. 折叠二级菜单；若无有意义的紧凑形态，则跳过此步。
3. 将二级菜单收入共享三点菜单。
4. 折叠一级菜单，例如「章节」变为「章」、「列表/表格」仅保留图标。
5. 若仍无法容纳，将一级菜单收入共享三点菜单。
6. 再处理在线人数和当前页面摘要等低优先级信息。

展开使用相反顺序，并保留统一的宽度缓冲，避免窗口在临界宽度往返时反复抖动。搜索展开期间暂停重新计算，且必须先隐藏页面右侧菜单，再展开输入框。
