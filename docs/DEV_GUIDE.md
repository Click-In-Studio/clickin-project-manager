...
### Production 顶部工具栏折叠规则

所有带顶部工具栏的 production 页面由 `AppShell` 统一管理折叠状态。Backstage 文本与通知/个人入口保留既有固定规则；从搜索开始的页面控件则按实际占用宽度推进。页面组件只声明控件层级和各阶段的呈现方式，不得自行增加响应式断点或另一套折叠状态。

- Stage 1：视口窄于 `1280px` 时隐藏 Backstage 文本。
- Stage 2：视口窄于 `1024px` 时隐藏通知与个人入口。

控件分为两级：

- **一级菜单（primary control）**：页面核心入口或视图切换，例如「章节」和「列表/表格」。
- **二级菜单（secondary control）**：依附于一级菜单或当前视图的设置，例如构作表格视图的「切换视图」和「列设置」。

内容驱动的折叠阶段固定为 `[0, 3, 4.1, 4.2, 5, 6]`。Stage 7 不存在，项目名与页面名组成的当前页面摘要在所有阶段都必须保留。只有当前阶段仍无法容纳时才进入下一阶段：

| Stage | 共享行为 | 剧本页 | Cue 页 | Cue 设置页 | 构作页 |
|-------|----------|--------|--------|------------|--------|
| 0 | 完整显示 | 完整工具栏 | 「分享 / 跳转至 行 页 段落 / 导出 / 设置」完整显示 | 「新建」在顶部工具栏单行显示 | 完整工具栏 |
| 3 | 搜索框折叠为图标 | 无额外变化 | 无额外变化 | 无额外变化 | 无额外变化 |
| 4.1 | 处理二级控件 | 无额外变化 | 「行 / 页 / 段落」按钮折叠为「跳转」菜单；「分享 / 设置」折叠为「设置」菜单 | 无额外变化 | 表格模式的视图与列设置收入共享三点菜单 |
| 4.2 | 折叠一级控件及页面特定二级控件 | 「章节 / 角色 / 编辑 / 显示」使用短标签，排练模式使用短标签，在线头像最多显示 2 个 | 可见性标签收入「激活」菜单；触发器只显示当前 Cue 名称并限制为 `118px`；「插入 Cue」移至底部 | 「新建」移至筛选标签同一行的最右侧 | 「列表 / 表格」只显示图标 |
| 5 | 收纳一级控件 | 章节、角色、编辑、显示、导出收入共享三点菜单 | 跳转、导出、设置收入共享三点菜单；设置内含「分享当前 cue 表」与「Cue 表设置」 | 无额外变化，「新建」不收入共享三点菜单 | 列表/表格切换收入共享三点菜单 |
| 6 | 处理剩余低优先级控件 | 在线成员收入共享三点菜单 | 无额外变化 | 无额外变化 | 无额外变化 |

角色页复用「列表/表格」控件的 Stage 4.2/5 行为。Cue 设置页沿用「项目名 / Cue」摘要，摘要右侧依次显示分隔线与返回 Cue 页的入口，并在页面内容的筛选标签上方显示「设置」标题。页面没有对应层级的控件时直接跳过该阶段，但不得改变后续阶段的编号。

可伸缩或横向滚动的工具栏区域必须用 `data-production-toolbar-flex-content` 标记其完整内容容器。`AppShell` 按该容器的固有宽度及外边距计算占用空间，不能使用已经被压缩或裁切后的外层宽度；Cue 可见性标签依赖此规则参与折叠测量。

带内部滚动区的 production 页面必须使用 `PRODUCTION_PAGE_SCROLL_ROOT_CLASS`（`h-full min-h-0 overflow-hidden`）占满 `AppShell` 已分配的 workspace 高度，不得再次用 `100vh` 计算页面高度。`AppShell` 会在底部导航出现时自动压缩 workspace；页面内容只保留自身的单一滚动区，避免 workspace 背景产生第二条滚动条。

展开使用相反顺序，并保留 `16px` 的统一宽度缓冲，避免窗口在临界宽度往返时反复抖动。搜索展开期间暂停重新计算，且必须先隐藏页面右侧菜单，再展开输入框。

只要页面已有控件收入共享三点菜单，折叠后的搜索入口也必须随之收入该菜单并固定为第一项；从菜单内展开搜索时，输入框与结果继续留在该菜单内。若没有其他已收纳控件，则搜索继续以独立图标显示，不单独生成三点菜单。
