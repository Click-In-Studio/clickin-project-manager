...
- **分享链接一律走有状态 token 间接层**（照 `asset_share_link`：short id PK + 随机 bearer token UNIQUE + `expires_at` / `one_time` / `revoked_at`），实体 id 不进分享 URL；每次兑现都查行，所以单条撤销、到期与项目出口策略即时生效。「知道 id ≠ 有权访问」。单次链接的“一次”是一次显式浏览器兑换，不是一次 HTTP 请求：落地页的 GET 不消费（避免聊天预览 / 安全扫描误吞），用户点击后原子占用并签发只存于 HttpOnly cookie 的随机会话秘密，库里只存哈希；媒体元数据 / Range / 下载共用该会话，30 分钟无活动失效、兑换后最长 8 小时，续用仍逐次检查链接行与项目策略。
...
- **剧本写侧三条路径共用一套行写与收尾**（#635）：对 `script` / `script_version` / `script_character` / `character_version` 的行级 INSERT / UPDATE / DELETE 只写在 `lib/script/script-row-tx.ts`，patch / 批量写 / 导入 / 标记修复都改调它，不再各抄一份 SQL；写事务收尾一律 `finalizeMarkerInvariantsInTx`（先从标记同步 `scene_version`、再归属回填、结构变了 bump revision——顺序是硬的，场次同步顺手立 scene identity 锚，反过来撞 FK）。`scene_version` 是标记的**派生读模型**，任何路径不得直写（导入的 `upsertScenes` 只剩 identity 锚用途）。护栏：`tests/script/write-version-content.test.ts` Group C 同一份内容经两条路径落库逐行比对。
