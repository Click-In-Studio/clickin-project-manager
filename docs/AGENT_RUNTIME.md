...
## 设计原则（新工具 / 新 skill 前先读）

这一节是产品侧 AI 设计的定谳，散在各 issue / PR 里容易被重新发明；改动前先看原出处。

- **能干的助理，不是万能许愿机**（#47 讨论定）：助理的品格是不懂就问、合作工作，不是尝试独立完成整活。工程签名——① 自治单位是**批次**不是整活，批次边界是合作点（确认卡 / 攒批提问 / 日志 checkpoint）；② 指示 skill 写**升级协议**（分诊 + 问人），不写案例大全——特殊情况是开集，硬套最接近的规则静默出错比问一句贵；③ 给 AI **工具箱清单**（有哪些自由度）而非用法手册；④ **丢弃也是裁决、裁决权在人**——输入里不进产物的东西必须有明账；⑤ 用户裁决即判例，沉淀到可引用的媒介（导入日志等）后同类一致适用、不重复问；⑥ **AI 视角 = 用户视角、误用靠明确反馈不靠隐藏**——给 AI 的世界视图（树 / 列表）与用户看到的一致，怕拿错 id 就在错误消息里指路，不把东西藏起来。propose / preflight / 审批卡是这一哲学在写面的体现。**面向用户的手册不要照搬这些措辞**（那是内部设计语言，搬过去就是训话）。
- **工具调用权限门原则**（2026-08-16）：① 调用点个人**有**权限键但标记敏感 → 聊天栏内确认（allow-once / deny）后执行；② 调用点个人**没有**权限键 → 不论是否敏感一律先提示「没有权限执行该操作」、**直接拦截不执行**，随即弹审批 modal（权限键固定为该调用点所需键、附本次调用详情）——走治理层正式申请，不是聊天栏能就地放行的确认。审批通过后自动重放调用是未来项，现在不做。这条与工具**可见性**分层是两个正交的轴，不要合并成一套逻辑。
- **语境不是权限**：production-context / 页面 UI 上下文只提供语境，工具内部照判 `hasEffectiveGrant`；多钥匙域用族内显式权限查询工具（见下「多钥匙域」），不往注入里放权限清单。
- **注入包裹标签必须在服务端真边界净化**（PR #295）：`<clickin-instructions>` 等包裹可被用户内容伪造提前闭合；净化点是 stream 路由的 `neutralizeInboundMessage`（客户端净化可被直构 API 绕过）；「可信脚手架」体内嵌了用户可控字段（doc.title）就不再可信，整段豁免前先问这段里有没有用户可写字段；工具结果里的用户自由文本读回模型前也要净化。别靠标签名保密当防护。
- **无人值守写允许，边界在 skills 内部**：注册表 `Def.unattended` 缺省 deny；写必进 `agent_mutation`；**撤销永远是人的动作，不做自动撤销**（详见下文两节）。
- **记忆自动管线绝不扩大信息可见面**：个人会话蒸馏的记忆不自动晋升到团队域——自动记忆导致的权限泄漏无责任人，不可接受（M3 暂停待议）。
- **召回按族不按单工具**：`tool-catalog` 每工具标 `family`，命中整族入面；动词类 tag 不进目录（宾语才是路由信号）；兜底是 `find_tools` 搜索不是目录。
- **剧本方言 id 往返协议**（`lib/script/script-dialect.ts`）：最小 diff 靠「序列化带 `[b:<id>]`、改写保留 id」的格式纪律，**明确否决对齐算法**（block_id 是 cue / tag / page_map 的锚点资产，模糊匹配错一次即静默断锚）；不定长方言必须转义感知扫描；改方言文法必须同批改序列化器 / 解析器 / `SCRIPT_DIALECT_NOTE` 三处并保住「serialize → parse → diffState 空 patch」测试。
- **文件即事实**：base prompt 六件套与工具注册表同源（有测试守）；不在 prompt 里追加更正块去纠正文件——两句互斥的话每轮都送给模型。
- **页面感知**：某页面对应的工具上线就回 `PAGE_SUGGESTIONS` 加一行，建议必须诚实（背后真有工具兑现）；不在表里的路由什么都不附带。
