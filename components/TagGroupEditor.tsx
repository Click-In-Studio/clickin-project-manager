...
function GroupCard({
  ...
  const handleReorderOptions = async (orderedOptions: TagOption[]) => {
    const previousOptions = group.options;
    onUpdate({ ...group, options: orderedOptions });
    try {
      await Promise.all(orderedOptions
        .filter(option => previousOptions.find(item => item.id === option.id)?.sortOrder !== option.sortOrder)
        .map(option => apiFetch(
          `${BASE_PATH}/api/production/${productionId}/tag-groups/${group.id}/options/${option.id}`,
          "PUT",
          { sortOrder: option.sortOrder },
        )));
    } catch {
      onUpdate({ ...group, options: previousOptions });
    }
  };
      ...
      {group.type === "exclusive" ? (
        <div className="space-y-1">
          <TagFormatOptionList
            ...
            onSplitAfterOption={canEdit ? handleSetLyricSplit : undefined}
            optionColor={option => pendingColors[option.id] ?? option.color}
            defaultOptionId={group.defaultOptionId}
            onColorChange={canEdit ? handleColorInput : undefined}
            onDefaultOption={canEdit ? handleSetDefault : undefined}
            onDeleteOption={canEdit ? handleDeleteOption : undefined}
            onReorder={canEdit ? ordered => void handleReorderOptions(ordered as TagOption[]) : undefined}
          />
          {canEdit && (
            <AddOptionForm
