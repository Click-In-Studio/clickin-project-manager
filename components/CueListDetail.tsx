  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div>
        ...
        {!showAddUser && (
          <button style={addBtnStyle} onClick={() => { setShowAddUser(true); setUserSearch(""); }}>
            + 添加成员
          </button>
        )}
        {showAddUser && (
          <div style={{ marginTop: 4, display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            {userSearch === "" ? (
              ...
              <button key={m.userId} disabled={saving}
                style={{ border: "1px solid var(--line)", borderRadius: 7, padding: "6px 10px", fontSize: 12, background: "var(--surface-2)", color: "var(--ink)", cursor: saving ? "default" : "pointer", textAlign: "left" }}
                onClick={async () => {
                  await postCollaborator({ type: "user", userId: m.userId });
                  setShowAddUser(false);
                  setUserSearch("");
                }}>
                <span>{m.name}</span>
                {m.roles.length > 0 && (
                  <span style={{ fontSize: 10, color: "var(--muted)", marginLeft: 6 }}>{m.roles.slice(0, 2).join("、")}</span>
                )}
              </button>
            ))}
