...
async function requireManage(req: NextRequest, productionId: string) {
  ...
  if (!(permCtx.isAdmin || await hasGrant(permCtx.userId, productionId, "org_dept", "*", "members", "create")) && !(permCtx.isAdmin || await hasGrant(permCtx.userId, productionId, "org_dept", "*", "*", "create")))
