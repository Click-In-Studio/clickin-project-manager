...
import { POLICY_OFF } from "@/lib/perm/policy-keys";
...
export async function PUT(req: NextRequest, ctx: Ctx) {
  ...
  // #469：wiki 公开开关参与 canViewWiki 判定，关掉即收窄全体可见文档的读面（打开是加权，不踢）
  if (res.changed.some((c) => c.key === "policy.wiki_public_enabled" && c.to === POLICY_OFF)) kickRevokedStreams(id);
