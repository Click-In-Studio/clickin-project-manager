...
import { listProductionRolesWithPermissions, createProductionRole } from "@/lib/perm/role-db";
...
async function requireGate(req: NextRequest, productionId: string, sub: string, verb: "view" | "create" | "edit" | "delete") {
  ...
  const ok = verb === "view"
    ? await hasEffectiveGrant(access.permCtx, productionId, "member", "*", "overrides", "edit")
    : await hasEffectiveGrant(access.permCtx, productionId, "role", "*", sub, verb);
