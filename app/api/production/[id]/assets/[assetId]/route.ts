...
// 判别联合：error 与 permCtx 二选一，调用方不需要 `!` 断言。
async function checkAccess(req: NextRequest, id: string): Promise<
  | { error: Response; permCtx?: undefined }
  | { error?: undefined; permCtx: PermissionContext }
> {
  const session = getSession(req.cookies);
  if (!session) return { error: Response.json({ error: "未登录" }, { status: 401 }) };
  const access = await getProductionPermissionContext(session.userId, session.isAdmin, id);
  if (!access) return { error: Response.json({ error: "权限不足" }, { status: 403 }) };
  return { permCtx: access.permCtx };
}

export async function GET(req: NextRequest, ctx: Ctx) {
  const { id, assetId } = await ctx.params;
  const { permCtx, error } = await checkAccess(req, id);
