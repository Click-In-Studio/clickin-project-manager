...
import { getWiki, duplicateWiki } from "@/lib/wiki/content";
...
// 都过了才写；三门全跑在 duplicateWiki 之前（write-before-authz 不许）。
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  const source = await getWiki(wikiId, productionId);
  if (!source) return Response.json({ error: "文档不存在" }, { status: 404 });
  ...
  if (!node) return Response.json({ error: "文档不存在" }, { status: 404 });
