...
import { updateTagOption, deleteTagOption } from "@/lib/script/script-block-tag-db";
...
export async function PUT(req: NextRequest, ctx: { params: Promise<{ id: string; groupId: string; optionId: string }> }) {
  ...
  if (!await hasEffectiveGrant(permCtx, id, "tag_group", groupId, "options/name", "edit")) {
...
export async function DELETE(_req: NextRequest, ctx: { params: Promise<{ id: string; groupId: string; optionId: string }> }) {
  ...
  const { permCtx, isArchived } = access;
  if (isArchived) return Response.json({ error: "已归档的项目不可修改" }, { status: 403 });
  if (!await hasEffectiveGrant(permCtx, id, "tag_group", groupId, "options", "delete")) {
    return Response.json({ error: "权限不足" }, { status: 403 });
  }
