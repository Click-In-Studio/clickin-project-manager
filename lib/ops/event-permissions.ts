...
import { hasAnyEffectiveGrant, hasEffectiveGrant, type GrantActor } from "../perm/grant-check";
...
/**
 * Can add or edit a specific task.
 * Primary: production_member_grant(task, taskId, 'edit'+).
 * 绑定 event 时 event 的 details@edit 级联可编（无绑定 task 无此通道）。
 */
...
import { listGrantedResourceIds } from "../perm/grant-check";
