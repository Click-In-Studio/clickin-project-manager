...
// 退役——168 键六批迁移完毕（见 lib/perm/permission-migration-ledger.ts RETIRED 清单）。
// 判定统一走树模型：
//   行判定  = hasGrant / hasEffectiveGrant（lib/perm/grant-check.ts，production_member_grant 表）
//   六步链  = canAccessNode（lib/perm/grant-template.ts，区间三表 + SENSITIVE 三态）
 ...
 * `node:dept/<D>/notes@create` 的通配形正是导演类模板键（见 lib/production/templates），
...
//   lib/production/templates/*.ts 的 roles.names
