 ...
 *    （lib/production/templates/*.ts）发了但判定端没门查的键属于另一类欠账（模板 > 判定的
 *    粒度差），不在这里补齐。
 *
...
export const PAGE_PERMISSION_SCOPES = {
  ...
  reports: new Set<string>([
    ...
    // 资源类型），判定端在 lib/ops/event-permissions.ts 有门
  ...
  assets: new Set<Permission>([
    // 新建素材条目。五条上传路由（route / presign / presign-part /
    // presign-multipart / relay-part，末条 #457 归队）查的都是 asset/*/*@create，
    // 而此前激活面只列了下面那枚 file@create（那是「给已有
