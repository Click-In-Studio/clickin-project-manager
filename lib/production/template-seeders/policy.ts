/**
 * 项目模版 · 策略 slot（见 `lib/production/production-template.ts` 文件头）。
 *
 * 载荷是**覆盖项**：只写与代码默认不同的键，其余取 `lib/perm/policy-keys.ts` 的 defaultValue。
...
import { ensureProductionPolicies } from "../../perm/policy-db";
import { policyDef, isLegalValue } from "../../perm/policy-keys";
