...
import { isRootNode, isSensitiveNode } from "../perm/grant-template";
import { loadDeptTree, collectAncestors } from "../perm/dept-db";
import {
...
/** 与 ApprovalNodeClass 同一个类型：治理域三态只有一处定义，见 lib/approval/approval-stages.ts。 */
...
// 级名、顺序与展示文案都在 lib/approval/approval-stages.ts —— 那个模块不碰 pg，前端也引得动。
...
// ─── 各级人选查询 ─────────────────────────────────────────────────────────────
// 导出给模版流引擎（lib/approval/approval-flow-engine.ts）复用：模版节点的处理人来源
