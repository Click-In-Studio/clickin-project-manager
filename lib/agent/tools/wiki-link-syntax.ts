...
export const WIKI_DIALECT_NOTE =
  ...
  "task（任务：文档任务列表项经「同步任务」建出的 production 任务，渲染成「标题 · 状态」；id 只能来自已有的任务引用或任务工具结果，不要编造）。" +
  ...
  "③ 空行——独占一行的 `&nbsp;` 是编辑器里的一个空段落。相邻两个同类列表之间必须隔一行 `&nbsp;`，" +
  "只隔空行会被 Markdown 并成一个列表；读到它原样保留。" +
  "④ 富表格——复杂表格用 :::table / :::endtable，内含 :::row / :::endrow，行内含 :::cell / :::endcell；" +
  "每个边界独占段落，前后空行。cell 可带 header=true、colspan=2、rowspan=2、colwidth=200（跨列用逗号，0=自动）、align=center；" +
  "未写属性表示普通格、跨度1、自动宽。格内仍用完全相同的 Markdown 方言，图片和引用不可改写成 HTML；" +
  "代码块里的边界只是代码，字面边界段落转义首个冒号。合并格的覆盖位置不另写空格；每行逻辑列数相同。" +
  "简单表格仍可用 GFM。不把无表头的图片排版表强加表头，不用空格对齐源码列。" +
