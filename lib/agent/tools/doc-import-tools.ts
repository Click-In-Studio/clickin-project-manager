...
假设要版本化记入日志，读到反例就修订。
outline 标了**栅格化 / 抽取不完整**的页（扫描件、拍照 pdf），文本工具读不到：
用 doc_page_ocr 按页识别（快档先；结果提示需要升档的页再用 tier: "full"
只跑那些页）。OCR 是识别文本，可能有错字漏行——写入前把关键处（人名、
场次号、含数字的句子）引原文问用户核对，不要当作文本层同等可信。
...
批内省着花：**同一区间只 doc_read 一次**，propose 直接凭已读结果写，不要
为"核对"再读一遍；一批的量按一个场景/小节取齐，特长的场景拆两批。
...
- **落点纪律**：导入的默认授权只有**插入**。任何触及既有块的操作（替换/
  删除/改写）超出导入授权，必须用户显式圈定范围才可做。
- **写入工具怎么选**：往**空白/新建场景**批量落正文，优先 rewrite——它
  直接吃方言文本（与读回形态一致，无 JSON 换算错位面），空场景没有可
  破坏的既有块；**碰既有内容的段**才限 edit_blocks 精修（insert 数组
  顺序=文档顺序，成功后按序返回新块 id 可作下批锚点）。
...
- **分块粒度：尽可能细**——一句台词一块；**歌词一律逐行成块**（轮唱/对唱
  必拆，整首歌也不例外——不要"整段 8 行用 | 续行塞一块"）。块是评论/cue/
  标签的锚粒度，粗块会让后续工作锚不住；台词粒度拿不准，开工问一次成判例；
- 一块可挂多个说话人（重唱/齐说），每个说话人可带 annotation——
  **annotation 是泄压阀**：「（与甲同时）」「（O.S.）」「（如果没有
  trapdoor）」这类不合文法的东西，角色归角色、怪话进 annotation。
  双栏对唱/复调（两组人同时各唱各的）也用它：各组分块，后组标注
  「（与上一块同时）」；
- 兜不住的进块级 stage_comment，再兜不住=问人；
- **空场景的 [空] 块是占位锚**：第一批写入时直接 update 它为正文
  （换上说话人/内容），或以它为锚往后插——不要绕开它也不要困惑。
