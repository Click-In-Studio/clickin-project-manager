import remarkTables from "../../editor/remark-tables";
...
export async function renderNotifyDoc(md: string, resolve: RefResolver): Promise<NotifyDoc> {
  ...
  const processor = unified().use(remarkParse).use(remarkGfm).use([...REMARK_CJK_PLUGINS]).use(remarkTables).use(remarkBreaks);
  const normalized = normalizeTableHtml(md);
  const tree = processor.runSync(processor.parse(normalized), { value: normalized }) as unknown as MdNode;
