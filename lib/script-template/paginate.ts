...
export type Page = { pageNum: number; items: PlacedItem[]; /** 本页首个正文块所在场次的标签（页眉用） */ sceneLabel: string };
...
export function paginate(items: LayoutItem[], opts: PaginateOptions): PaginateResult {
  ...
  const flush = () => {
    ...
    pages.push({ pageNum, items: cur, sceneLabel: curLabel });
    pageNum++;
    cur = [];
    used = 0;
  ...
  for (const item of items) {
    if (item.kind === "sceneHeading") {
      ...
      if (used > 0 && used + h > contentHeight) flush();
    ...
    if (!hasBlockOnPage) curLabel = activeSceneLabel;
