...
-- ── Grant Template（权限REST化 批A，总表 §0.7）─────────────────────────────────
-- 行集模板：模板行=资格，生效需 self-confirm 写 resource_grant 个人行。
-- 全局模板 production_id IS NULL 按 holder_name（角色名，'*'=所有成员）；
-- 演出级按 holder_id，有行则覆盖全局；dept 模板行沿 dept 树向下生效。
