...
> {
  ...
  // ── 本地补丁 #2（见 vendor/openclaw/VENDOR.md）──────────────────────────
  // 从当前 transcript 续跑而不追加用户消息：进程重启后恢复中途 run 的入口
  // （最后一条消息必须是 user / toolResult；assistant 结尾的 transcript 本来
  // 就是"已完成"，由调用方判定为 idle）。与 executeTurn 同构，只是换成
  // runAgentLoopContinue、不触发 before_agent_start（那是给新 prompt 的钩子）。
  async continueTurn(): Promise<AssistantMessage> {
    ...
    try {
      const turnState = await this.createTurnState();
      const last = turnState.messages[turnState.messages.length - 1];
      if (!last) {
        throw new AgentHarnessError("invalid_state", "Cannot continue: empty transcript");
      }
      if (last.role === "assistant") {
        throw new AgentHarnessError("invalid_state", "Cannot continue: transcript ends with an assistant message");
      }
